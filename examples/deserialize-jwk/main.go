...
func main() {
	...
	*  These are indexed by their KeyID, if present, else by a short int */
	...
	key, _ := jwks.JWK2KeyPublic(jwk)
	fmt.Println(key)

	/* Or we can get an object that will unmarshal JSON into Keys */

	parser := &jwks.JWKPublic{} // No getter for this as it wouldn't do anything
	...
	type MyType struct {
		Foo   int            `json:"foo"`
		MyJWK jwks.JWKPublic `json:"myjwk"`
	}
